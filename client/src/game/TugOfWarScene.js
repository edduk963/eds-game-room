...
export class TugOfWarScene extends Phaser.Scene {
  ...
  init(data) {
    ...
    this.onGameStarted = data.onGameStarted || (() => {});
    this.opponentName = data.opponentName || 'Opponent';
    this.myName = data.myName || 'You';
  }

  preload() {
    this._mkRect('tow-player',   36, 18, 0x5cffd4);
    this._mkRect('tow-bullet',    4, 12, 0xffffff);
    this._mkRect('tow-invader',  28, 18, 0xff77aa);
    this._mkRect('tow-ufo',      40, 16, 0xffcc44);
    this._mkRect('tow-civilian', 28, 16, 0x88ddff);
    this._mkRect('tow-debris',   18, 18, 0x886644);
    this._mkRect('tow-decoy',    20, 20, 0xffaa00);
  }
  ...
  create() {
    ...
    this.player = this.physics.add.sprite(W / 2, H - 40, 'tow-player');
    this.player.setCollideWorldBounds(true);

    this.bullets   = this.physics.add.group({ defaultKey: 'tow-bullet', maxSize: 30 });
  ...
  _spawnInvaderWave() {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        const e = this.invaders.create(xPad + c * dx, yTop + r * dy, 'tow-invader');
  ...
  _spawnDebris() {
    ...
    const d = this.debris.create(x, -20, 'tow-debris');
  ...
  _spawnCivilian() {
    ...
    const c = this.civilians.create(fromLeft ? -30 : W + 30, y, 'tow-civilian');
  ...
  _spawnDecoy() {
    if (this.gameOver) return;
    const x = rngRange(this.rng, 30, W - 30);
    const d = this.decoys.create(x, -20, 'tow-decoy');
  ...
  _spawnUfo() {
    ...
    const fromLeft = this.rng() < 0.5;
    const u = this.ufos.create(fromLeft ? -40 : W + 40, rngRange(this.rng, 50, 100), 'tow-ufo');
